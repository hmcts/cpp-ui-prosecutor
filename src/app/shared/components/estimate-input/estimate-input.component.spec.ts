import { Component, DebugElement, forwardRef, Type } from '@angular/core';
import { ComponentFixture, fakeAsync, TestBed, tick } from '@angular/core/testing';
import { AbstractControl, FormsModule, NG_VALUE_ACCESSOR, NgForm } from '@angular/forms';
import { By } from '@angular/platform-browser';
import { EstimateInputComponent } from './estimate-input.component';
...
describe('estimate-input', () => {
  ...
  const initTestWith = (C: Type<TestEstimateInput>) =>
    fakeAsync(() => {
      ...
      estimate = form.control.get('estimate');
  ...
  const setTimeValues = (values: { weeks?: string; days?: string; hours?: string; minutes?: string }) => {
  ...
  describe('when only minutes are used', () => {
    @Component({
      ...
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" aria-describedby="identifier"> </estimate-input>
        </form>
      `,
      imports: [EstimateMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateMinutesComponent));
  ...
  describe('when hours are enabled', () => {
    @Component({
      ...
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" hours-enabled aria-describedby="identifier">
          </estimate-input>
        </form>
      `,
      imports: [EstimateHoursMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateHoursMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateHoursMinutesComponent));
  ...
  describe('when days and hours are enabled', () => {
    @Component({
      ...
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" hours-enabled days-enabled aria-describedby="identifier">
          </estimate-input>
        </form>
      `,
      imports: [EstimateDaysHoursMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateDaysHoursMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateDaysHoursMinutesComponent));
  ...
  describe('when weeks, days and hours are enabled', () => {
    @Component({
      ...
      `,
      imports: [EstimateWeeksDaysHoursMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateWeeksDaysHoursMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateWeeksDaysHoursMinutesComponent));
  ...
  describe('when days are enabled', () => {
    @Component({
      ...
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" days-enabled aria-describedby="identifier">
          </estimate-input>
        </form>
      `,
      imports: [EstimateDaysMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateDaysMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateDaysMinutesComponent));
  ...
  describe('when weeks are enabled', () => {
    @Component({
      ...
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" weeks-enabled aria-describedby="identifier">
          </estimate-input>
        </form>
      `,
      imports: [EstimateWeeksMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateWeeksMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateWeeksMinutesComponent));
  ...
  describe('when weeks and hours are enabled', () => {
    @Component({
      ...
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" weeks-enabled hours-enabled> </estimate-input>
        </form>
      `,
      imports: [EstimateWeeksHoursMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateWeeksHoursMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateWeeksHoursMinutesComponent));
  ...
  describe('when weeks and days are enabled', () => {
    @Component({
      ...
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" weeks-enabled days-enabled> </estimate-input>
        </form>
      `,
      imports: [EstimateWeeksDaysMinutesComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateWeeksDaysMinutesComponent {
      model: number;
    }

    beforeEach(initTestWith(EstimateWeeksDaysMinutesComponent));
      ...
      template: `
        <form>
          <estimate-input
            ...
            hours-enabled
            days-enabled
            (fieldFocus)="focus($event)"
            (fieldBlur)="blur($event)"
          >
          </estimate-input>
        </form>
      `,
      imports: [EstimateAnyComponent, FormsModule, EstimateInputComponent]
    })
    class EstimateAnyComponent {
      model: number;
      focus: () => void;
      blur: () => void;
    }

    beforeEach(initTestWith(EstimateAnyComponent));
  ...
  describe('when minimum or maximum value validators are provided', () => {
    let component: EstimateInputComponent;

    @Component({
      selector: 'estimate-days-hours-minutes-input',
      template: `
        <form>
          <estimate-input name="estimate" [ngModel]="model" aria-describedby="identifier" [minMinutesValue]="10">
          </estimate-input>
        </form>
      `,
      imports: [EstimateDaysHoursMinutesComponent, FormsModule, EstimateInputComponent]
    ...
    beforeEach(() => {
      component = fixture.debugElement.query(By.directive(EstimateInputComponent)).componentInstance;
    });
    ...
    it('error suffix is `minute` when value entered is less than minumum of 1', fakeAsync(() => {
      component.minMinutesValue = 1;
      tick();
      fixture.detectChanges();
      setTimeValues({ minutes: '0' });
      const errors = form.control.controls['estimate'].errors;
      expect(errors.minMinutesEstimate.suffix).toEqual('minute');
    }));

    it('error suffix is `minutes` when value entered is less than minumum of 10', () => {
      setTimeValues({ minutes: '9' });
      const errors = form.control.controls['estimate'].errors;
      expect(errors.minMinutesEstimate.suffix).toEqual('minutes');
    });
  });
});
